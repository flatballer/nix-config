#!/usr/bin/env bash
# Build a NixOS SD card image for Raspberry Pi 4.
#
# Requirements:
...
echo "==> Building NixOS SD image for Raspberry Pi 4..."
echo "    (requires aarch64-linux remote builder from Mac)"
nix build "${FLAKE_DIR}#nixosConfigurations.pi4.config.system.build.sdImage" \
  --print-build-logs \
  "$@"
